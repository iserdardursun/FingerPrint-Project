...
% Last Modified by GUIDE v2.5 09-Jul-2019 15:23:28
...
for i=1:r
    ...
    for j=1:k
        ...
        if strcmp(uniq(i), first(j))
            p = size(temp_struct);
            if p==0
                temp_struct = table2struct(yeni_data(j, 2:7));
            else
                temp_struct = [temp_struct; table2struct(yeni_data(j, 2:7))];
            end
        end
    end
...
if (maxim<0.48) 
    ...
    t_path = char(struct2cell(table2struct(person(x,5))));   
    fprintf(['done!\n>> Found fingerprint! Welcome ' name '!\n']);
    set(handles.t_header, 'string', ['Authenticated, hello ' name '!']);
    axes(handles.axesp)
    imshow(t_path)
    drawnow();
    [y,fs]=audioread('C:\Users\ASUS\Desktop\ses.mp3');
    sound(y,fs)
end;
