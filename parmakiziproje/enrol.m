...
% Last Modified by GUIDE v2.5 09-Jul-2019 14:53:09
...
res=imread('C:\Users\ASUS\Desktop\olcsan.jpg');
axes(handles.axes2)
imshow(res)
...
profilpath=get(handles.profil_path,'string');
...
rec1 = struct('Name', name, 'Age', age, 'FID1', id1, 'FID2', id2,'Profil',profilpath);
...
if path1~=0
   ...
   for i=1:m
       for j=1:n
            %minu1(i,j)=(minu1(i,j)*2)+500;
            temp=num2str(minu1(i,j));
            temp_data = encrypt(temp,reps,key);%3x3 matrisimiz 
            gecici = reshape(temp_data.',1,[]);%3x3 matrisi stringe çevirme tek boyutlu dizi halinde
            gecici={gecici};
            string_data(i,j)=gecici;
        end
   end
...
if path2~=0
    ...
    minu2 = struct('ID', id2, 'X', string_data(:, 1), 'Y', string_data(:, 2),...
                   'Type', string_data(:, 3), 'Angle', string_data(:, 4), ...
                   'S1', string_data(:, 5), 'S2', string_data(:, 6));
end;
...
function profil_path_Callback(hObject, eventdata, handles)
% hObject    handle to profil_path (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of profil_path as text
%        str2double(get(hObject,'String')) returns contents of profil_path as a double


% --- Executes during object creation, after setting all properties.
function profil_path_CreateFcn(hObject, eventdata, handles)
% hObject    handle to profil_path (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% handles    structure with handles and user data (see GUIDATA)
[filename1, pathname1] = uigetfile();  
set(handles.profil_path, 'string', [pathname1 filename1]);
...
% handles    structure with handles and user data (see GUIDATA)
[filename1, pathname1] = uigetfile();  
set(handles.t_f2, 'string', [pathname1 filename1]);