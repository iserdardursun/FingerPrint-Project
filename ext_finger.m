% PARMAK ÝZÝ RESMÝNDEN ÖZELLÝK ÇIKARMA

function [ ret ] = ext_finger( img, display_flag )
    if nargin==1; display_flag=0; end%Eðer 1 girdi varsa display_flag deðiþkenine 0 atýyoruz.
    if ndims(img) == 3; img = rgb2gray(img); end%ndims:vektördeki sayýlarýn boyutunu bulur 
    block_size_c = 24; YA=0; YB=0; XA=0; XB=0;
% Arttýrma -------------------------------------------------------------
    if display_flag==1; fprintf('>> enhancing... '); end
    yt=1; xl=1; [xr,yb]=size(img); 
    
    % 1-55 satýrlarýndaki piksel sayýsý 8'den küçük olan deðerler bulunur.
    % Eðer 8 den küçükse bu deðerler 255 yapýlýr.
    % Parmak izinin solundaki alanlar temizlenir. Bu iþlem tekrarlanýr.
    % 225 satýr, 200 sütun ve 1-75 kolon
    for x=1:55
        if numel(find(img(x,:)<200)) < 8%numel:eleman sayýsýný verir.
           img(1:x,:) = 255;
           yt=x;
        end
    end
    ...
    [ binim, mask, cimg, cimg2, orient_img, orient_img_m ] = f_enhance(img);%Filtreleme iþlemi
    if display_flag==1
        imshow(binim);title('Filtrelenmiþ ve Binarize Görüntü');
    end;
% Maskeleme iþlemi -------------------------------------------------------------
    if display_flag==1; fprintf('tamam!\n>> maskeleme yapýlýyor. '); end
    mask_t=mask;
    mask=mask_t;
    inv_binim = (binim == 0);
    thinned =  bwmorph(inv_binim, 'thin',Inf);%bwmorph:maskeleme metodu
    mask_t=mask;
    ...
    lower_t=0.1;%alt deðerimiz
    [v,y]=min(cimg);%burada cimg matrisinden her sütundaki en küçük deðerleri alýp v deðiþkenine atýyor ve bunlarýn indekslerini de y deðiþkenine atýyor. 
    [dt1,x]=min(v);
...
% Minutiae bulmak --------------------------------------------------------
    if display_flag==1; fprintf('Tamam!\n>> Minutiae bulunuyor... '); end
    minu_count = 1;
    minutiae(minu_count, :) = [0,0,0,0,0,1];
    min_path_index = [];
    % görüntü üzerinde döngü ve minutiae noktalarýný bulmak ve sýnýr için
    % belirli pikselleri yoksayma
    for y=20:size(img,1)-14
        for x=21:size(img,2)-21
            if (thinned(y, x) == 1) % sadece piksel beyaz olursa devam et.
                % hesaplamak 
                CN = 0; sx=0; sy=0;
                for i = 1:8
                  t1 = p(thinned, x, y, i);
...
% Sahte Minutiae filtreleme ------------------------------------------------
    if display_flag==1; fprintf('tamam!\n>> Sahte Minutiae Filtreleniyor...'); end
    minu_count = minu_count -1;
    t_minutiae = [];
    t_minu_count = 1;
    t_mpi = [];
...
% Minutiae döndürme ---------------------------------------------------------
    if display_flag == 1
        ...
        elseif minutiae(i, 3) == 3                  %Çatallanma
          if minutiae(i, 4) > pi
            for k = y1-2: y1 + 2
              for l = x1-2: x1 + 2
        ...
        %subplot(1,2,1), subimage(img), title('Orjinal resim')
        if display_flag==1
